...
package net.mcreator.sobelow.world.biome;
...
public class MoltenBelowstoneBiome extends SoBelowModElements.ModElement {
	@ObjectHolder("so_below:molten_belowstone")
	public static final CustomBiome biome = null;
	public MoltenBelowstoneBiome(SoBelowModElements instance) {
		super(instance, 42);
	}
	...
	public void init(FMLCommonSetupEvent event) {
		BiomeDictionary.addTypes(biome, BiomeDictionary.Type.SPOOKY, BiomeDictionary.Type.DEAD, BiomeDictionary.Type.NETHER);
	}
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(1f).scale(0.5f).temperature(0.4f).precipitation(Biome.RainType.RAIN)
					.category(Biome.Category.NETHER).waterColor(-3407872).waterFogColor(-3407872)
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(MoltenRockBlock.block.getDefaultState(),
							DeepstoneBlock.block.getDefaultState(), DeepstoneBlock.block.getDefaultState())));
			setRegistryName("molten_belowstone");
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION,
					new CustomTreeFeature()
							.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(Blocks.OBSIDIAN.getDefaultState()),
									new SimpleBlockStateProvider(Blocks.AIR.getDefaultState()))).baseHeight(3)
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == MoltenRockBlock.block.getDefaultState().getBlock()
							|| ground == DeepstoneBlock.block.getDefaultState().getBlock())
							&& (ground2 == MoltenRockBlock.block.getDefaultState().getBlock()
									|| ground2 == DeepstoneBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), DeepstoneBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.OBSIDIAN.getDefaultState().getBlock()
					|| blockType == Blocks.AIR.getDefaultState().getBlock() || blockType == MoltenRockBlock.block.getDefaultState().getBlock()
					|| blockType == DeepstoneBlock.block.getDefaultState().getBlock();
		}
